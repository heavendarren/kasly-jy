 ...
 *
 */
package de.platform.kaslyjy.cockpits.setup;
...
import de.platform.kaslyjy.cockpits.constants.KaslyjyCockpitsConstants;
 ...
 */
@SystemSetup(extension = KaslyjyCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(KaslyjyCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(KaslyjyCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						KaslyjyCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/kaslyjycockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
